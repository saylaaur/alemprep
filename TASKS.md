...
Тех-долг P0 закрыт (T1–T3c ✅). T6 ✅. Дальше:

1. **T7 — Режим «Полный пробник»** (функционал, в работе)
2. **T10 — Пайплайн генерации контента (math)** — материалы привезены, поэтому подняли вверх
3. **T5 — Мобильная адаптация** (полировка)
4. **T11 — Расширение схемы под графики/картинки** (нужно для ~10–20% задач — отложено)
5. **T12 — Информатика: сидинг тем + пайплайн** (отложено — позже, чтобы качественнее)
6. **P2** — error tracking, тесты
...
### T6. Страница «Прогресс» — ✅ СДЕЛАНО (verified)
