...
**Стратегия от 10.09:** [ROADMAP до 1 июня 2027](ROADMAP.md) — регулярная работа двух школ, проверенный KK и кейс для технических стажировок/будущих стартапов. Ближайший формат — [сопровождаемый пилот](docs/pilot/SUPERVISED_PILOT.md), с отдельным допуском SP0–SP7; полный технический план сохраняется. В C00a включить подтверждённую обрезку счётчиков до 1 000 строк вместо 4 646 и несовпадение каталога/тренажёра. Дата первого урока ещё не согласована школами.
Основной документ: [полный production-план](docs/production/README.md), [архитектура](docs/superpowers/specs/2026-09-09-production-pilot-architecture.md), [промпт Terra](docs/production/IMPLEMENTER_PROMPT.md), [журнал исполнения](docs/production/EXECUTION_LOG.md). **Следующая задача C00a (P0 — казахский контент), затем C00b; E01/E02 продолжаются как обязательные технические ворота.** Детальные планы C00a–C00c и E01–P03 заменяют порядок старых блоков; код и внешние допуски отмечаются раздельно. Ни одна будущая задача не выполнена фактом написания плана.

**P0 от 09.09:** read-only инвентаризация production: 4 646 опубликованных RU, **0 KK**, 59 черновиков суммарно. Для двух сельских школ казахский — приоритет приёмки. [План C00a–C00c](docs/superpowers/plans/2026-09-09-03-content-and-language.md): проверить покрытие и ошибки загрузки → экономный перевод существующих задач через Google Cloud Translation NMT → проверка казахского и предмета человеком → публикация принятой партии. Перевод хранится в БД; при открытии урока API перевода не вызывается. Генерация новых задач и массовый платный LLM-review не являются обязательными этапами. План обновлён, сам дефицит ещё не исправлен.

Исторический контекст 08.09:
Спецификация: [условия и границы пилота](docs/pilot/SCOPE.md).
Порядок работ и условия допуска: [план подготовки школьного пилота](docs/superpowers/plans/2026-09-08-rural-schools-pilot.md).
Блок 0 в работе: [паспорт проверки](docs/pilot/RELEASE.md). Для safety SHA проходили типы/линтер, 485 тестов и Webpack build; права БД частично подтверждены. Стандартный build, привязка домена к коммиту и полный пользовательский сценарий ещё не приняты. [Аудит и кабинет учителя](docs/pilot/AUDIT_AND_REPORTING.md) обязательны до первого занятия; визуализации — Desmos API после проверки лицензии и языков. Текущий порядок — E01–P03 выше. Старый порядок ниже сохраняется как исторический бэклог и не определяет приоритет школьного запуска.
...
### T10. Пайплайн генерации контента (math) — ✅ КОД ГОТОВ (verified). Демо-запуск ждёт ANTHROPIC_API_KEY

**Зачем.** Материалы привезены: 82 PNG математических задач в
`~/Desktop/unt recources/math unt 1` и `math unt 2`. Цель — превратить их в
**~240–400 переписанных задач** (3–5 вариантов на эталон), залить в БД с
`is_published=false`, после ручной проверки публиковать. Это закрывает Sprint 3 в ROADMAP.

**Решения, согласованные на POC (см. этот чат / POC выше):**
- **Только математика** в v1 (информатика отложена — T12).
- **Задачи с графиками/схемами пропускаем** (помечаем как `skip: 'graph'` на этапе
  транскрипции; расширение схемы под картинки — T11).
- Качество — как в POC: LaTeX в формулах, правдоподобные дистракторы,
  пошаговый разбор, `source: 'ai_rewritten'`.
- Никакого дословного копирования из НЦТ-пробников.

**Файлы.**
- `scripts/transcribe-questions.ts` — PNG → reference JSON через Claude vision.
- `scripts/generate-variants.ts` — reference JSON → N вариантов через Claude Sonnet API.
- `scripts/references/math-{batch}.json` — выход транскрипции.
- `scripts/generated/math-{batch}.json` — итоговые варианты.
- `scripts/lib/schema.ts` — zod-схемы под наш `QuestionBody` (см. `types/db.ts`).
- `scripts/insert-batch.sql` (генерится скриптом) — готовый SQL `INSERT … is_published=false`.
- `package.json` — npm-скрипты `gen:transcribe` и `gen:variants`.
- `.env.local` — новый ключ `ANTHROPIC_API_KEY` (важно: **API-биллинг отдельный от Claude Pro**,
  предупреди пользователя; залогируй ориентировочную стоимость каждого вызова).

**Пайплайн.**
1. **Транскрипция** (раз на партию): итерируем PNG в указанной папке → для каждого
   отправляем картинку Claude (vision) → reference JSON `{topic_slug, type, body, correct,
   explanation, source_file}` или `{skip: 'graph' | 'unsupported', reason}`.
   Графики/схемы фильтруются автоматически.
2. **Генерация** (раз на эталон): берём reference JSON → промпт «измени числа/контекст,
   сохрани сложность и тему, верни N вариантов в строгом JSON» → zod-валидация →
   сохраняем в `generated/`.
3. **Заливка**: генерим SQL `INSERT` с `is_published=false`, пользователь выполняет в
   Supabase Studio.
4. **Ручная проверка** (партиями 20–30): просматриваем, помечаем `is_published=true`.

**Подсказки.**
- `@anthropic-ai/sdk` (npm install).
- Модель — последний Claude Sonnet (быстро + достаточно качественно для текстовой математики).
- Use structured output (JSON mode) + zod-валидация на выходе.
- Логировать input/output tokens + примерную стоимость для каждой партии.
- topic_slug'и сверять со списком в БД (`subjects/topics`) — если эталон попал в тему,
  которой нет, скрипт должен явно сообщить и предложить добавить миграцию.

**Готово, когда.** На демо-партии (5 эталонов math) скрипты транскрибируют, генерируют
по 3 варианта каждый, проходят zod-валидацию; итоговый `insert-batch.sql` готов; примерная
стоимость партии записана в лог. `/verify` зелёный.

**Промпт:**
```
Построй пайплайн генерации контента по T10 в TASKS.md. Два скрипта:
scripts/transcribe-questions.ts (PNG → reference JSON через Claude vision; задачи с
графиками/схемами помечаются skip: 'graph') и scripts/generate-variants.ts (reference
JSON → N вариантов через свежий Claude Sonnet, zod-валидация). Используй
@anthropic-ai/sdk, добавь npm-скрипты gen:transcribe и gen:variants. Ключ —
ANTHROPIC_API_KEY в .env.local (предупреди про отдельный API-биллинг). Логируй
стоимость партии в долларах. Сделай демо-партию на 5 эталонов из
~/Desktop/unt recources/math unt 1 — проверь, что zod пропускает и собирается
scripts/insert-batch.sql с is_published=false. Сначала покажи план (plan mode) и
структуру файлов, пиши код после моего «ок». В конце /verify.
```

---

### T11. Расширение схемы под графики/картинки (отложено)

**Зачем.** В пробниках есть задачи с графиками функций, схемами цепей и т.п. Сейчас
наша JSON-схема `body` чисто текстовая — такие задачи скрипт-генератор (T10) пропускает.
Чтобы покрыть их (~10–20% объёма), нужно добавить опциональное поле для картинки.

**Что сделать (черновик).** Добавить в `QuestionBody` опциональное `image?: { url: string;
alt: string; caption?: string }` (или вариант — массив `images`). Хранение — Supabase Storage
(уже доступно). Обновить `types/db.ts`, `MathText`/рендер вопроса (показ картинки над опциями),
zod-схему в T10 (теперь принимает `image`), скрипт транскрипции (выгружать PNG задачи в
storage и проставлять `url`).

**Когда.** После T10 — когда увидим, сколько реально задач теряется без поддержки картинок.

---

### T12. Информатика: сидинг тем + пайплайн (отложено)

**Зачем.** В БД сейчас есть только темы математики. Информатика-материалы (40 PNG)
лежат в `~/Desktop/unt recources/informatics 1`, но без topic_slug'ов их нельзя залить.

**Что сделать.** Новая миграция `00XX_seed_informatics_topics.sql` — 5–7 тем
по спецификации НЦТ (системы счисления, алгоритмы и блок-схемы, программирование,
БД, сети, логика). Затем запустить T10-пайплайн на папке `informatics 1` с теми же
правилами (text+code+LaTeX, графики пропускаем).

**Когда.** После T10 — отложили специально, чтобы пайплайн отшлифовать на математике
и потом сделать информатику качественнее.
