...
**Актуальный приоритет с 08.09.2026 — пилот в двух сельских школах.**
...
Блок 0 в работе: [паспорт проверки](docs/pilot/RELEASE.md). Типы/линтер и 485 тестов проходят, Webpack build проходит; права БД частично подтверждены. Стандартный build, привязка домена к коммиту и полный пользовательский сценарий ещё не приняты. [Аудит и кабинет учителя](docs/pilot/AUDIT_AND_REPORTING.md) обязательны до первого занятия; визуализации — Desmos API после проверки лицензии и языков. Дальше выполнять блоки этого плана. Старый порядок ниже сохраняется как исторический бэклог и не определяет приоритет школьного запуска.
