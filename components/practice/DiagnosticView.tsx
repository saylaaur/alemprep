...
import { startDiagnostic, finishDiagnostic } from '@/lib/supabase/diagnostic-actions';
import type { ExamContext } from '@/lib/supabase/queries';
import { DIAGNOSTIC_BLOCK_COUNT, EXAM_FIRST_SUBJECT, QUESTION_POINTS, scoreAnswer } from '@/lib/exam';
...
type Props = { second: SecondSubject; locale: string };

export function DiagnosticView({ second, locale }: Props) {
  ...
  // Предупреждаем при уходе со страницы во время диагностики (без localStorage-резюма).
  useEffect(() => {
  ...
  // Клавиатурные шорткаты — как в пробнике: цифры выбирают вариант, стрелки листают.
  useEffect(() => {
    ...
    return (
      <div className="mx-auto max-w-lg px-6 py-16 text-center">
        ...
        <p className="mt-3 text-muted-foreground">
          {t('subtitle', { count: DIAGNOSTIC_BLOCK_COUNT * 2 })}
        </p>
  ...
  if (phase === 'result' && result) {
    return <DiagnosticResultScreen blocks={blocks} answers={answers} locale={locale} result={result} t={t} tExam={tExam} />;
  }
...
type ResultProps = {
  ...
  t: ReturnType<typeof useTranslations<'diagnostic'>>;
  tExam: ReturnType<typeof useTranslations<'exam'>>;
};

function DiagnosticResultScreen({ blocks, answers, locale, result, t, tExam }: ResultProps) {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-8 px-4 py-8 sm:px-6">
      ...
      <Button asChild size="lg" className="w-full shadow-primary">
        <Link href="/dashboard">{t('goToPlanButton')}</Link>
      </Button>
    </div>
  );
}
