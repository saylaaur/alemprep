...
- Проверка типов: `npm run typecheck` (оба tsconfig — приложение и `scripts/`); юнит-тесты: `npm test` (Vitest)
- Прод-сборка: `npm run build` (dev-сервер типы НЕ проверяет — см. «Грабли»)
...
- **Dev-сервер на SWC НЕ проверяет типы.** Сайт может работать на localhost, а `next build` — падать. Перед деплоем ВСЕГДА гонять `npm run typecheck` (покрывает и `scripts/`) или `npm run build`.
...
Неактуальные грабли (починено, не удивляйся старым упоминаниям):
- Тренажёр (`PracticeView`) и пробник локализованы через next-intl.
- `app/auth/callback` больше не хардкодит `/ru`: локаль едет в параметре `next` через OAuth, разбор — в `lib/auth-redirect.ts` (дефолт ru, защита от open redirect).
...
## Работа с Claude Code (рабочее соглашение)
...
- **Ворота качества перед «готово».** Dev-сервер на SWC НЕ проверяет типы, поэтому всегда:
  - `npm run typecheck` — обязан проходить (0 ошибок; покрывает и `scripts/`);
  - `npm run lint` — без новых предупреждений;
  - `npm test` — юнит-тесты Vitest (`lib/*.test.ts`) зелёные;
