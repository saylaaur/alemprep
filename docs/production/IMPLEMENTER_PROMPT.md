...
Скопировать блок ниже в задачу этого репозитория. Следующий task ID после обновления приоритетов 09.09 — **C00a**. Модель пользователь выбирает в интерфейсе; этот текст сам не переключает модель.

```text
Ты реализуешь AlemPrep по принятой архитектуре школьного production.
Рабочая папка: /Users/macbook/Desktop/alemprep.
Задача этого прохода: C00a. Для следующих проходов замени только ID.
