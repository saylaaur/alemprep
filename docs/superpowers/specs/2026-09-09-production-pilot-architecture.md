...
Уточнение 10.09: эта архитектура описывает полный самостоятельный школьный продукт. Первый [сопровождаемый пилот](../../pilot/SUPERVISED_PILOT.md) допускает ручную операторскую сводку вместо полного учительского UI, с отдельной приёмкой SP0–SP7. Инварианты серверной выдачи, доступа, атомарного сохранения/аудита и восстановления сохраняются; SP-ready не равен завершению E–P. Долгосрочные приоритеты и кейс до лета — в [ROADMAP](../../../ROADMAP.md).

Уточнение 09.09: казахский — первый язык приёмки пилота. При нулевом опубликованном KK-банке C00a–C00c имеют P0. Существующие RU-вопросы переводятся заранее через Google NMT как машинные черновики, с предметной и языковой проверкой человеком; повторная генерация задач не нужна. Эта подготовка может идти до L01, а публикация ранней партии — после E02 в существующей схеме. При переходе к L01/C01 пары и сведения о проверке переносятся в версионированную модель. Платный перевод не вызывается из ученического запроса; 100% утверждённой программы доступно на KK без скрытого RU fallback.
